  ...
  const betaModal = isBetaModalOpen && createPortal(
    <div className="fixed inset-0 z-[99999] bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
      <motion.div
        ...
        className="bg-[var(--bg-panel)] rounded-3xl border border-[var(--border-strong)] max-w-sm w-full shadow-2xl p-6 relative flex flex-col items-center text-center overflow-hidden"
      >
        <button
          onClick={() => setIsBetaModalOpen(false)}
          className="absolute top-4 right-4 p-2 rounded-full hover:bg-[var(--bg-surface)] transition-colors text-[var(--text-muted)] hover:text-[var(--text-base)] z-10"
        >
          <X className="w-5 h-5" />
        </button>

        <h3 className="text-[1.3rem] font-bold text-[var(--text-base)] mb-4 mt-2 flex items-center justify-center flex-wrap gap-2">
        ...
        <div className="flex flex-col w-full gap-3">
          <button
            onClick={() => setIsBetaModalOpen(false)}
            className="w-full py-3 bg-[var(--bg-surface)] hover:bg-[var(--border-strong)] text-[var(--text-base)] rounded-xl font-medium transition-colors text-sm"
          >
          ...
          {onFeedbackRequest && (
            <button
              ...
              className="w-full py-3 bg-[var(--color-sec)] text-white rounded-xl font-medium hover:opacity-90 transition-opacity text-sm shadow-sm"
            >
              Envie um feedback
            </button>
          )}
